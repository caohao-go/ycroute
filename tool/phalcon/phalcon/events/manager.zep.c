...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC))) {
		...
		if (zephir_is_true(&_1$$4)) {
			...
			ZVAL_LONG(&_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 39, &_2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 40, handler, &_3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0 TSRMLS_CC)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(&_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 39, &_1$$5);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$5, 3);
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(&_5$$6, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 40, &_6$$7, &_7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), &eventType, &newPriorityQueue TSRMLS_CC);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 41, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue TSRMLS_CC))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 42, &_2$$5, &_1$$5);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0 TSRMLS_CC)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 43, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 44, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0 TSRMLS_CC)) {
		...
		if (_5$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 43, &eventName, source, data, &_6$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 44, &fireEvents, &event);
			zephir_check_call_status();
