					...
					if (Z_TYPE_P(&value) != IS_ARRAY) {
						do {
							if (ZEPHIR_IS_LONG(&type, 1)) {
								ZVAL_LONG(&_4$$12, 10);
								ZEPHIR_CALL_FUNCTION(&castValue, "intval", &_5, 5, &value, &_4$$12);
								zephir_check_call_status();
								break;
							}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, query) {
	...
	if (Z_TYPE_P(&statement) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 6, this_ptr, &statement, &sqlStatement, bindParams, bindTypes);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Db_Adapter_Pdo, convertBoundParams) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "preg_match_all", NULL, 7, &bindPattern, &sql, &matches, &_0);
	...
	if (zephir_is_true(&_1)) {
		...
		ZVAL_STRING(&_5$$3, "?");
		ZEPHIR_CALL_FUNCTION(&boundSql, "preg_replace", NULL, 8, &bindPattern, &_5$$3, &sql);
		zephir_check_call_status();
	} else {
